...
def fermion_single_double_excitation_pool(n_qubit, hamiltonian=None, fermi_qubit_transform=bravyi_kitaev):
    ...
    """
    if hamiltonian == None:
        hamiltonian = general_single_generator(n_qubit)+uccgsd_double_generator(n_qubit)
...
def upccgsd_pool(n_qubit, fermi_qubit_transform=bravyi_kitaev):
    ...
    hamiltonian = general_single_generator(n_qubit)+upccgsd_double_generator(n_qubit)
    from Utilities.Iterators import iter_terms_in_fermion_operator
...
def upccgsd_generator(n_qubit, anti_hermitian=True):
    if n_qubit % 2 != 0:
        raise ValueError('The total number of spin-orbitals should be even.')

...
def grid_transverse_field_ising(n_col,n_row,field,spin_coupling=1):
    ...
    for i in range(n_row-1):
        for j in range(n_col):
            spin_coupling_hamiltonian+=QubitOperator("Z"+str(i*n_col+j))*QubitOperator("Z"+str((i+1)*n_col+j))
    ...
    init_list=[2*i for i in range(n_qubit//2)] #TODO
    energy_obj=EnergyObjective(hamiltonian,n_qubit,init_block=HartreeFockInitBlock(init_list))
...
def full_connected_transverse_field_ising(n_qubit,field=1,spin_coupling=1):
    ...
    energy_obj=EnergyObjective(hamiltonian,n_qubit,init_block=get_transverse_field_init_block(n_qubit))
...
def haldane_chain_s_1(n_qubit,h1,h2,J=1):
    ...
    from .SpinChainInitBlock import get_haldane_chain_s_1_init_block
    energy_obj=EnergyObjective(hamiltonian,n_qubit,init_block=get_haldane_chain_s_1_init_block(n_qubit))
    return energy_obj
...
def heisenberg_model(n_qubit,periodic=True):

    spin_coupling_hamiltonian=QubitOperator()
    for i in range(n_qubit-1):
        spin_coupling_hamiltonian+=QubitOperator("Z"+str(i))*QubitOperator("Z"+str(i+1))
        spin_coupling_hamiltonian+=QubitOperator("X"+str(i))*QubitOperator("X"+str(i+1))
        spin_coupling_hamiltonian+=QubitOperator("Y"+str(i))*QubitOperator("Y"+str(i+1))
    if periodic:
        spin_coupling_hamiltonian+=QubitOperator("Z"+str(n_qubit-1))*QubitOperator("Z"+str(0))
        spin_coupling_hamiltonian+=QubitOperator("X"+str(n_qubit-1))*QubitOperator("X"+str(0))
        spin_coupling_hamiltonian+=QubitOperator("Y"+str(n_qubit-1))*QubitOperator("Y"+str(0))

    hamiltonian=spin_coupling_hamiltonian

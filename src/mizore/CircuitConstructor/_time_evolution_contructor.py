...
from ._analytical_rts_constructor import AnalyticalRTSConstructor
...
def save_benchmark_for_paths(paths, delta_n=1, trotter_steps=None, task_manager=None):
    """
    Implement same functionality as generate_benchmark_for_compare_from_paths,
    but allow the run in the paths have different Hamiltonians,
    will also save a json of result in each path
    """
    if trotter_steps is None:
        trotter_steps = [1]
    for path in paths:
        circuit_list_list=[]
        circuit_list_0 = read_circuits_from_file(path, delta_n=delta_n)
        with open(path + "/run_info.json", "r") as f:
            log_dict = json.load(f)
        delta_t=log_dict["delta_t"]
        delta_t1=delta_n*delta_t
        init_circuit = circuit_list_0[0]
        with open(path + "/energy_obj.pickle", "rb") as f:
            hamiltonian = pickle.load(f).hamiltonian

        circuit_list_list.append(circuit_list_0)

        for n_trotter_step in trotter_steps:
            trotter_circuits = generate_trotter_benchmark_circuits(
                init_circuit, n_trotter_step, hamiltonian, delta_t1, len(circuit_list_0))
            circuit_list_list.append(trotter_circuits)

        fidelity_list = generate_benchmark_for_compare(
            circuit_list_list, hamiltonian, delta_t1, task_manager=task_manager)
        result_dict={}
        result_dict["delta_n"]=delta_n
        benchmark_dict={}
        benchmark_dict["this_run"]=fidelity_list[0]
        index=1
        for n_trotter_step in trotter_steps:
            benchmark_dict[n_trotter_step]=fidelity_list[index]
            index+=1
        result_dict["benchmark"]=benchmark_dict
        with open(path+ "/benchmark.json", "w") as f:
            json.dump(result_dict,f)


def draw_benchmark_with_error_from_paths(paths,max_time):
    with open(paths[0] + "/run_info.json", "r") as f:
        info_dict = json.load(f)
    delta_t=info_dict["delta_t"]
    with open(paths[0] + "/benchmark.json", "r") as f:
        result_dict = json.load(f)
    delta_n=result_dict["delta_n"]
    delta_t1=delta_t*delta_n
    fidelity_list_dict={}
    benchmark_dict=result_dict["benchmark"]
    max_index=math.ceil(max_time/delta_t1)
    for key in benchmark_dict.keys():
        fidelity_list_dict[key]={}
        for i in range(max_index):
            fidelity_list_dict[key][i*delta_t1]=[]

    for path in paths:
        with open(path + "/benchmark.json", "r") as f:
            result_dict = json.load(f)
        assert delta_n==result_dict["delta_n"]
        benchmark_dict=result_dict["benchmark"]
        for key in benchmark_dict.keys():
            for i in range(max_index):
                fidelity_list_dict[key][i*delta_t1].append(benchmark_dict[key][i])
    max_min_avg_dict={}
    from copy import copy
    for key in fidelity_list_dict.keys():
        benchmark_dict=fidelity_list_dict[key]
        max_fidelity_list=[]
        min_fidelity_list=[]
        avg_fidelity_list=[]
        for key_1 in benchmark_dict.keys():
            local_fidelity_list=benchmark_dict[key_1]
            max_fidelity_list.append(max(local_fidelity_list))
            min_fidelity_list.append(min(local_fidelity_list))
            avg_fidelity_list.append(sum(local_fidelity_list)/len(local_fidelity_list))
        max_min_avg_dict[key]={"max":copy(max_fidelity_list),"min":copy(min_fidelity_list),"avg":copy(avg_fidelity_list)}
    print(fidelity_list_dict)
    print(max_min_avg_dict)
    draw_max_min_avg_dict_and_save(paths,max_min_avg_dict,delta_t1)

def draw_max_min_avg_dict_and_save(paths,max_min_avg_dict,delta_t):
    import matplotlib
    import matplotlib.pyplot as plt
    ...
    time_list=[delta_t*i for i in range(len(max_min_avg_dict["this_run"]["avg"]))]
    for key in max_min_avg_dict.keys():
        ax.plot(time_list, max_min_avg_dict[key]["avg"], '-',
                   color="dodgerblue",label=key)
        ax.fill_between(time_list, max_min_avg_dict[key]["min"], max_min_avg_dict[key]["max"], alpha=0.2)
    for path in paths:
        plt.savefig(path+'/comprison_1.png', bbox_inches='tight')

...
def generate_benchmark_for_compare(circuit_list_list,hamiltonian,delta_t):
    ...
    for i in range(len(circuit_list_list)):
        ...
        for step in range(1,list_n_circuit[i]):
            fidelity_list[step]=abs(get_inner_two_circuit_product(circuit_list_list[i][step],benchmark_circuits[step]))
...
def generate_trotter_benchmark(init_circuit,n_trotter_step,hamiltonian,delta_t,n_circuit):
    circuit_list=generate_trotter_benchmark_circuits(init_circuit,n_trotter_step,hamiltonian,delta_t,n_circuit)
...
def draw_run_status_figure(path):
    ...
    first_trotter_gate_use=TrotterTimeEvolutionBlock(hamiltonian,n_trotter_step=1,evolution_time=0.1).get_gate_used()[gate_name]
    ...
    fig = plt.figure()
    ax = fig.add_subplot()
    lns1=ax.plot(evolution_time_list, quality_list, '-',color="dodgerblue",label="Distance")

    ax2 = ax.twinx()
    lns2=ax2.plot(n_gate_time_list, n_gate_list, '-',color="orange",label=gate_name+" count: Adaptive")
    lns3=ax2.plot([0,end_time], [first_trotter_gate_use,first_trotter_gate_use], '-',color="red",label=gate_name+" count: 1st Trotter ")
    ax.grid()
    ax.set_xlabel("Time")
...
def generate_benchmark_for_compare_from_paths(paths):
    ...
    for n_trotter_step in [1,2]:
        ...
        trotter_circuits=generate_trotter_benchmark_circuits(init_circuit,n_trotter_step,hamiltonian,delta_t,max_n_circuit)
        circuit_list_list.append(trotter_circuits)
        label_list.append("Trotter "+str(n_trotter_step))
    fidelity_list_list=generate_benchmark_for_compare(circuit_list_list,hamiltonian,delta_t)
    return fidelity_list_list,label_list,delta_t

def draw_benchmark_for_compare_from_paths(paths):
    fidelity_list_list,label_list,delta_t=generate_benchmark_for_compare_from_paths(paths)
    min_fidelity=1
